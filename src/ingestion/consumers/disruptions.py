"""TfL ``disruptions`` Kafka → Postgres consumer entrypoint.

Wires :class:`ingestion.consumers.RawEventConsumer` to
:class:`contracts.schemas.DisruptionEvent` and ``raw.disruptions``.
"""

from __future__ import annotations

import asyncio
import os

from contracts.schemas import DisruptionEvent
from ingestion.consumers.event_consumer import RawEventConsumer
from ingestion.consumers.kafka import KafkaEventConsumer
from ingestion.consumers.postgres import RawEventWriter
from ingestion.observability import configure_logfire

__all__ = [
    "DISRUPTIONS_CONSUMER_GROUP_ID",
    "DISRUPTIONS_TABLE",
    "DISRUPTIONS_TOPIC_LABEL",
    "main",
]

DISRUPTIONS_CONSUMER_GROUP_ID = "tfl-monitor-disruptions-writer"
DISRUPTIONS_TABLE = "raw.disruptions"
DISRUPTIONS_TOPIC_LABEL = "disruptions"
_DISRUPTIONS_CLIENT_ID = "tfl-monitor-ingestion-disruptions-consumer"


async def _amain() -> None:
    configure_logfire(instrument_psycopg=True)

    bootstrap = os.environ.get("KAFKA_BOOTSTRAP_SERVERS", "")
    if not bootstrap:
        raise SystemExit("KAFKA_BOOTSTRAP_SERVERS is required")

    dsn = os.environ.get("DATABASE_URL", "")
    if not dsn:
        raise SystemExit("DATABASE_URL is required")

    async with (
        KafkaEventConsumer(
            DisruptionEvent.TOPIC_NAME,
            bootstrap_servers=bootstrap,
            group_id=DISRUPTIONS_CONSUMER_GROUP_ID,
            client_id=_DISRUPTIONS_CLIENT_ID,
        ) as kafka,
        RawEventWriter(dsn, table=DISRUPTIONS_TABLE) as writer,
    ):
        consumer = RawEventConsumer[DisruptionEvent](
            kafka_consumer=kafka,
            writer=writer,
            event_class=DisruptionEvent,
            topic_label=DISRUPTIONS_TOPIC_LABEL,
        )
        await consumer.run_forever()


def main() -> None:
    """Module entrypoint used by ``python -m ingestion.consumers.disruptions``."""
    asyncio.run(_amain())


if __name__ == "__main__":
    main()
