...
9. **[2026-05-15] `gh pr merge` from the main checkout fails when `main` is also checked out in a worktree**
   ...
   Do instead: invoke from a non-git cwd with `--repo` explicit — `cd /tmp && gh pr merge <N> --squash --delete-branch --repo hcslomeu/tfl-monitor`. The shell cwd is reset by the harness afterwards. Always confirm with `gh pr view <N> --json state,mergeCommit -q '{state, sha: .mergeCommit.oid[:7]}'` before assuming the merge landed.
