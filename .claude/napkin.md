...
8. **[2026-05-11] `gh pr merge --admin` is per-action gated by the hook — pre-confirm with the user**
   ...
   Do instead: before the first `--admin` merge, ask explicitly e.g. `merge todos os PRs com --admin` and wait for the user to repeat the `--admin` keyword. For stacked-PR force-pushes, ask `autorizado force-push` separately. Once authorized in-session, subsequent calls in the same session pass.
