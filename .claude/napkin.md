...
11. **[2026-05-19] `docker compose restart <svc>` does NOT re-read `env_file` — use `up -d --force-recreate --no-deps <svc>`**
    After URL-encoding `@` chars in `/opt/tfl-monitor/.env`'s `DATABASE_URL` to fix the Supabase pooler password parse, `docker compose restart api` reported success and `docker exec tfl-monitor-api-1 env | grep DATABASE_URL` showed the new value — but the running uvicorn process kept the OLD pre-encoded value cached in `os.environ`, because `restart` sends SIGTERM+SIGINT to the existing PID-1 and re-runs `CMD`, it does NOT re-evaluate compose's `env_file:` directive against the container's environment. The symptom was `/api/v1/status/live` continuing to time out with `psycopg.errors.OperationalError: failed to resolve host '<password-fragment>@<rest>'` 30 seconds after the `.env` rewrite, even though the file content was correct.
    Do instead: after editing any `.env` referenced via `env_file:` in `docker-compose*.yml`, run `docker compose -f <file> up -d --force-recreate --no-deps <svc>` — not `docker compose restart <svc>`. `up --force-recreate` tears the container down and rebuilds it from the compose spec, which re-reads `env_file` from disk; `restart` just sends a signal to the running process. Same trap applies to `environment:` literals built from `${VAR_NAME}` shell interpolation — recreate, never restart. Sanity check: `docker inspect <container> --format '{{range .Config.Env}}{{println .}}{{end}}' | grep <KEY>` shows what the container ACTUALLY has after recreate.
