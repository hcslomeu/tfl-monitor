...
9. **[2026-05-15] `gh pr merge` from the main checkout fails when `main` is also checked out in a worktree**
   Running `gh pr merge <N> --squash --delete-branch` from `/Users/humbertolomeu/tfl-monitor` aborted with `failed to run git: fatal: 'main' is already used by worktree at '/Users/humbertolomeu/tfl-monitor'`. Hit twice this session (PR #68 + PR #69 merges) — the parent checkout has `main` and gh's post-merge bookkeeping (`git checkout main && git pull && git branch -D <feature>`) trips on the existing worktree pin. Server-side the merge sometimes still lands (verify with `gh pr view <N> --json state`); when it doesn't, the local error blocks the call entirely.
   Do instead: invoke from a non-git cwd with `--repo` explicit — `cd /tmp && gh pr merge <N> --squash --delete-branch --repo hcslomeu/tfl-monitor`. The shell cwd is reset by the harness afterwards. Always confirm with `gh pr view <N> --json state,mergeCommit -q '{state, sha: (.mergeCommit.oid // "pending")[:7]}'` before assuming the merge landed — the null-safe slice keeps the filter from crashing while the PR is still open.
