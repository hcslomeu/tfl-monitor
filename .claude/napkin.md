...
10. **[2026-05-16] Dashboard rewrites can orphan wired fetchers without a build failure**
...
11. **[2026-05-16] `infra/README.md`'s "DNS already live" claim diverged from Cloudflare reality**
    On TM-F2 Phase 3.5 prod smoke, `dig +short tfl-monitor-api.humbertolomeu.com` returned EMPTY despite `infra/README.md:16` asserting "Cloudflare A record `tfl-monitor-api.humbertolomeu.com` → `13.41.145.33` (DNS only, gray cloud)". The Lightsail box itself is alive (`tcp/443 + tcp/80` both open from outside, `http://13.41.145.33/` returns Caddy's 308 HTTP→HTTPS redirect) and `infra/caddyfile.snippet` has the site block ready, but the matching Cloudflare record was either never created or got removed. Caddy can't ACME-HTTP-01 itself a cert until the record points at the IP, so curl against the apex returns `code 6 (Couldn't resolve host)` and the dashboard's `NEXT_PUBLIC_API_URL=https://tfl-monitor-api.humbertolomeu.com` resolves to nothing in the browser. Same plan PR even noted §2.3 "verify with `dig`; if missing, add the CNAME on Cloudflare" — that verification was never actually run.
    Do instead: any WP whose smoke step depends on a hostname **must** `dig +short <hostname>` and `nc -z <ip> 443` BEFORE running the env-var wiring or the curl preflight. README claims about DNS / TLS / Caddy site blocks are aspirational documentation, not state; treat them as "the runbook says this *should* be true" and verify against the live record. The fix is two manual steps on Cloudflare (Add record → A → `tfl-monitor-api` → `13.41.145.33` → DNS only / gray cloud → save) plus waiting ~30 s for Caddy to acquire the Let's Encrypt cert via HTTP-01; no GH-action or REST-API path from this repo today. After the record lands, `curl -i https://tfl-monitor-api.humbertolomeu.com/health` should return 200 within a minute.

12. **[2026-05-19] `PR merged` ≠ `deployed` — four silent GHA deploy failures hid behind a "✅ shipped" PROGRESS row for five days**
