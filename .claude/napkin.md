...
9. **[2026-05-15 / re-confirmed 2026-05-18] `gh pr merge --delete-branch` fails when `main` is checked out in another worktree**
   ...
   Do instead: invoke from a non-git cwd with `--repo` explicit — `cd /tmp && gh pr merge <N> --squash --delete-branch --repo hcslomeu/tfl-monitor`. If the failure already happened, finish cleanup manually: `git push origin --delete <branch>` for the remote, `ExitWorktree(action: "remove", discard_changes: true)` if you're inside the worktree, then `git pull --ff-only` to sync local main. Always confirm with `gh pr view <N> --json state,mergeCommit -q '{state, sha: (.mergeCommit.oid // "pending")[:7]}'` before assuming the merge landed.
